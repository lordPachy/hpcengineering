...
Heat::assemble()
{
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      // Saves the values of the old "solution" variable into solution_old_*
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                ...
                // Right - hand side
                // Time derivative
                cell_rhs(i) += (1. / delta_t) * //
                                       fe_values.shape_value(i, q) * //
                                       solution_old_values[q] * //
                                       fe_values.JxW(q);
                
                // Diffusion 
                cell_rhs(i) -= (1. - theta) * mu_loc * //
                                       scalar_product(fe_values.shape_grad(i, q), //
                                                      solution_old_grads[q]) * //
                                       fe_values.JxW(q);

                // Forcing term
                cell_rhs(i) += theta * f_new_loc * (1-theta) * f_old_loc;
  ...
  // Note that no boundary condition is applied,
  // as we have homogeneous Neumann
...
Heat::output() const
{
  ...
  // The entire solution is output, not only the owned parts
...
Heat::run()
{
  ...
  // We interpolate the initial conditions
  ...
  // Simulation loop. Tolerance never hurts
  while(time < T - 0.5 * delta_t)
  {
    ...
    // Recall that here we have a mess of inter-nodal communication
    solution = solution_owned;
    output();
  }
}