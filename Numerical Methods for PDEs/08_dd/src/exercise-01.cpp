...
main(int argc, char *argv[])
{
  ...
  while(n_iter < n_max_iter && solution_increment_norm > tolerance_increment){
    ...
    // This is not the integral norm, it's the l2 norm of the representation
    // The norms are equivalent (convergence is the same), though
    // this norm gives more weight to more dense regions
