...
{
  // This is a bijection going from boundary points in one domain
  // to the other
  ...
  // Note that this can only work if there is a 1-to-1 correspondance
  // between domains, and if other.solution[dof.second] is accessible.
  // The first condition is ok if the two domains have the same degree,
  // while the second is ok if the program is serial
  // In other cases, further investigation is needed
  std::map<types::global_dof_index, double> boundary_values;
...
Poisson2D::compute_interface_map(const Poisson2D &other) const
{
  ...
  // This is O(|sigma|^2), but with r-trees we can get it down to O(nlogn).
...
Poisson2D::apply_relaxation(const Vector<double> &old_solution,
                            const double         &lambda)
{
  // This weird construction minimizes the number of temporary vectors
  // used
  solution *= lambda;
