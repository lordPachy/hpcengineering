          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  ...
                  cell_matrix(i, j) += (mu_0_loc + mu_1_loc * 
                              solution_loc[q] * solution_loc[q]) *
              ...
              // -b(...)
              // pow is not used because solution_loc is complex
              cell_rhs(i) -= (mu_0_loc + mu_1_loc * 
                              solution_loc[q] * solution_loc[q]) *
                            solution_gradient_loc[q] * 
                            fe_values.shape_grad(i, q) * 
                            fe_values.JxW(q);
...
NonLinearDiffusion::solve_newton()
{
  ...
  while(n_iter < n_max_iters && residual_norm > residual_tolerance){
    ...
    if (residual_norm > residual_tolerance){
      ...
      solution = solution_owned; // Update ghost values
    } else {
      pcout << "Something" << endl;
    }
