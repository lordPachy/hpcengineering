...
The native Eigen direct solvers are based on the Choleski and LU factorization in the case in which the input matrix is symmetric or not, respectively.\
LDLT is a slight variation of Cholesky that looks like $$A = LDL^T$$ with $D$ diagonal, and $$\forall i\ L_{ii}=1 $$ where $D_{ii}$ are just the squares of the diagonal elements of the original Cholesky decomposition.\
The syntax for calling the `SparseLU` and `SimplicialLDLT` Eigen functions is similar to the one used for the iterative solvers `ConjugateGradient` and `BICGSTAB`:
...
We want to compare the `SparseLU` and `SimplicialLDLT` direct solvers with respect to the hand-made Conjugate Gradient method for solving the linear system $H x = b$, where $b$ is obtained by taking $x = (1,1,\ldots, 1)$ as exact solution.

Since the conditioning number of the Hilbert matrix is very large (and it grows with dimension), we observe that the direct solvers does not work. The resulting error is of order $10^3$.
...
We can note that Conjugate Gradient's residual highly depends on the last iterations. This is because it is a Krylov space method, and it (theoretically, in exact arithmetic) converges only when i == n.\
Note that when we allow i >= n, the performances should (theoretically, again) be comparable to those of a direct solver, since it is bound to converge at that iteration.
