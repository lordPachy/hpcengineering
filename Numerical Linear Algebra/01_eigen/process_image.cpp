...
int main(int argc, char* argv[]) {
  ...
  unsigned char tmp;
  MatrixXd grayscale_image_rotated(width, height);

  // Rotating the image
  for (int i = 0; i < height; i++){
    for (int j = 0; j < width; j++){
        grayscale_image_rotated(j, i)= grayscale_image(i, j);
    }
  }

  std::cout << "Done" << std::endl;

  // Adding -40 to darken the image
  MatrixXd darkening = MatrixXd::Constant(width,height, -40);
  grayscale_image_rotated += darkening;

  Matrix<unsigned char, Dynamic, Dynamic, RowMajor> grayscale_image_darkened(width, height);
  grayscale_image_darkened = grayscale_image.unaryExpr([](double val) -> unsigned char {
    return static_cast<unsigned char>(val);
  });

  
  // Save the grayscale image using stb_image_write
  const std::string output_image_path = "./01_eigen/output_grayscale_darkened.png";
  if (stbi_write_png(output_image_path.c_str(), height, width, 1,
                      grayscale_image_darkened.data(), height) == 0) {
      std::cerr << "Error: Could not save darkened grayscale image" << std::endl;
      
      return 1;
  }

  std::cout << "Darkened grayscale image saved to " << output_image_path << std::endl;
  return 0;
}