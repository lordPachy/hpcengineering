...
```
mpirun -n 4 ./test2 100 100 1 sol.mtx hist.txt -i gmres -p ssor -adds true -adds_iter 2
mpirun -n 4 ./test2 100 100 1 sol.mtx hist.txt -i gmres -p ssor -adds true -adds_iter 3
mpirun -n 4 ./test2 100 100 1 sol.mtx hist.txt -i gmres -p ssor -adds true -adds_iter 4
```
...
### Reconfiguration of LIS

Dowload the LIS library folder from `https://www.ssisc.org/lis/index.en.html` alternatively use git:

```
git clone https://github.com/anishida/lis
cd lis
```

Use the following commands for configuration and installation:

```
./configure --enable-f90 --enable-saamg
make
make check
```
